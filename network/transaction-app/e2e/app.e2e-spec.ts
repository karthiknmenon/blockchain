...
describe('Starting tests for transaction-app', function() {
  ...
  it('website title should be transaction-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('transaction-app');
    })
  });
  ...
  it('navbar-brand should be transaction-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('transaction-app');
    });
  });
